...
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own contest feedback" ON user_contest_feedback FOR DELETE USING (auth.uid() = user_id);
...
DECLARE v_contest contests %ROWTYPE;
v_feedback_exists BOOLEAN;
v_feedback_type TEXT;
BEGIN -- Lock the contest record for update to prevent race conditions
SELECT * INTO v_contest
FROM contests
WHERE id = p_contest_id FOR
UPDATE;
IF NOT FOUND THEN RAISE EXCEPTION 'Contest with ID % not found',
p_contest_id;
END IF;
